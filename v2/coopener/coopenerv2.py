...
def main(latitude, longitude, script, url, port, serialName="/dev/ttyAMA0", name="Coopener", filename="times.txt"):
  '''
  ...
  doorThread = Thread(target=doorWatch, args=(door, url, port)) # This is a separate thread which monitors for HTTP comms from SmartHome
...
def doorWatch(door, url, port):
  ...
  ip = get_ip_address() # Get the WiFi IP of the RasPi
  while(True): # This thread keeps running
    while(connected == False): # If connection is not made with SmartHome, then attempt to make it
      try:
        ...
        result = urllib.request.urlopen(url + ":" + port + "/handshake?shake=1&ip=" + ip).read()
...
if __name__ == "__main__":
  ...
  port = "5000" #SmartHome Server port
  ...
  t1 = Thread(target=main, args=(latitude, longitude, script, url, port, serialName, name, filename))
