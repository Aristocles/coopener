...
import json, os, re, serial, time, logging, argparse, sys
...
def main():
  ...
  '''
  if getProcess(os.path.basename(__file__)): exit() # If the script is already running, then exit
...
class Door(object):
  ...
  def getOpenTimeLeft(self): # Returns the number of seconds left to open the door
    now = datetime.now() # Get current date and time
